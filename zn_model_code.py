...
class ZnModel:
    def __init__(self, lattice_size, n_states, temperature, external_field=0):
        ...
        self.lattice = np.random.randint(0, self.n, (self.L, self.L))
...
lattice_size = 50
n_states = 2
temperature_range = np.linspace(5.0, 0.01, 500)
...
def update(frame):
    zn_model.T = temperature_range[frame]
    ...
    ax1.set_title(f"Lattice Configuration (T={zn_model.T:.2f})")
